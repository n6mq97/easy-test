...
import { getTests, getSections, addTest, getTestsBySection } from '@/lib/database';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (Array.isArray(body)) {
      ...
      for (const test of body) {
        if (test.section && test.question && Array.isArray(test.answers) && typeof test.correct === 'number') {
          const testId = addTest(test.section, test.question, test.answers, test.correct);
    ...
    if (!section || !question || !Array.isArray(answers) || typeof correct !== 'number') {
      return NextResponse.json({ error: 'Invalid test data' }, { status: 400 });
    }
    
    const testId = addTest(section, question, answers, correct);
    return NextResponse.json({ id: testId, section, question, answers, correct });
  } catch (error) {

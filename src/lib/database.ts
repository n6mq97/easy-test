...
export function addTest(section: string, question: string, answers: string[], correct: number) {
  const stmt = db.prepare('INSERT INTO tests (section, question, answers, correct) VALUES (?, ?, ?, ?)');
...
export function getSections(): string[] {
  const stmt = db.prepare('SELECT DISTINCT section FROM tests ORDER BY section');
  return stmt.all().map((row: any) => row.section) as string[];
}

// Функции для работы с результатами
export function saveResult(userId: number, testId: number, userAnswer: number, isCorrect: boolean) {
  const stmt = db.prepare('INSERT INTO results (user_id, test_id, user_answer, is_correct) VALUES (?, ?, ?, ?)');
...
export function getUserStats(userId: number): UserStats {
  ...
  return {
    ...stats,
    accuracy: stats.total_answered > 0 ? (stats.correct_answers / stats.total_answered * 100).toFixed(1) : '0',
    sectionStats
  };
}
...
const sampleTests = [
  {
    ...
    question: "Який алгоритм пошуку найкоротших шляхів працює тільки з невід'ємними вагами?",
    answers: [
      "Bellman-Ford",
      "Floyd-Warshall",
      "Dijkstra",
  ...
  {
    section: "1.1.5. Алгоритми на графах",
    question: "Який алгоритм найкраще підходить для побудови мінімального кістякового дерева через сортування ребер?",
    answers: [
      "Прим",
      "Дейкстра",
      "Крускал",
  ...
  {
    section: "1.1.5. Алгоритми на графах",
    question: "Скільки в середньому часу займає алгоритм Дейкстри (Dijkstra), якщо використовується купа?",
    answers: [
      "O(V²)",
      "O(V + E)",
      "O((V + E) log V)",
  ...
  {
    section: "1.1.5. Алгоритми на графах",
    question: "Який з наведених алгоритмів підходить для графів з негативними вагами?",
    answers: [
      "DFS",
      "Floyd-Warshall",
      "Dijkstra",
...
if (existingTests.length === 0) {
  ...
  sampleTests.forEach((test, index) => {
    try {
      ...
      console.log(`Додано тест ${index + 1}: ${test.question.substring(0, 50)}...`);
    } catch (error) {
